...
def get_patch_dataset(img_l, label_l, buffer_scale = 3, is_masking=False, batch_size = 1):
    ...
    dataset = dataset.map(lambda img_l, label_l, mask_l: tuple(tf.py_func(_read_py_function_1_patch, [img_l, label_l, mask_l], [tf.float32, tf.int32])), num_parallel_calls=4)
